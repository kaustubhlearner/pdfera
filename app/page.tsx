...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function previewStampAsset() {
      ...
      try {
        if (currentOverlayFile.type === "image/png" || currentOverlayFile.type === "image/jpeg" || currentOverlayFile.type === "image/svg+xml") {
          if (!cancelled) setStampAssetPreview(URL.createObjectURL(currentOverlayFile));
          return;
        }

        if (currentOverlayFile.type === "application/pdf" || currentOverlayFile.name.toLowerCase().endsWith(".pdf")) {
          const pdfjsLib = await import("pdfjs-dist/legacy/build/pdf.mjs");
          pdfjsLib.GlobalWorkerOptions.workerSrc = new URL(
            "pdfjs-dist/legacy/build/pdf.worker.min.mjs",
            import.meta.url
          ).toString();
          const pdf = await pdfjsLib.getDocument({ data: await currentOverlayFile.arrayBuffer() }).promise;
          const page = await pdf.getPage(1);
